...
function getTableStatus(model, cb) {
  ...
  function decoratedIndexDataCallback(err, data) {
    ...
    if (!err && data.length > 0) {
      data.forEach(function (index) {
        var i = indexHash[index.name] = indexHash[index.name] || {
          name: index.name,
          type: index.type,
          primary: index.primary,
          columns: []
        };
        i.columns.push(index.column);
      });

      data = [];
      Object.keys(indexHash).forEach(function (indexName) {
        data.push(indexHash[indexName])
      });
    }
  ...
  this.query('SELECT t.relname AS "table", i.relname AS "name", ' +
    'a.attname AS "column", am.amname AS "type", ix.indisprimary AS "primary" ' +
    'FROM pg_class t, pg_class i, pg_index ix, pg_attribute a, pg_am am ' +
    'WHERE t.oid = ix.indrelid AND i.oid = ix.indexrelid AND ' +
    'i.relam = am.oid AND ' +
    'a.attrelid = t.oid AND a.attnum = ANY(ix.indkey) AND ' +
...
PostgreSQL.prototype.getDefaultIndexName = function (model, propName) {
  return [self.table(model), self.column(model, propName), 'idx'].join('_');
};
...
PostgreSQL.prototype.addIndexes = function (model, actualIndexes, cb) {
  ...
  if (actualIndexes) {
    actualIndexes.forEach(function (i) {
      ...
      } // FIXME need to determine multi-column key order
    });
  }
  var aiNames = Object.keys(ai);

  // remove indexes
  // FIXME - This currently removes removed indexes but does not update modified indexes
  aiNames.forEach(function (indexName) {
    var i = ai[indexName];
    var propName = propNameRegEx.exec(indexName);

    if (i.info.primary || (m.properties[indexName], self.id(model, indexName))) return;
  ...
  indexNames.forEach(function (indexName) {
    ...
    if (!found) {
      ...
      var columns = Object.keys(i.keys).map(function (col) {
          return self.escapeName(self.column(model, col));
        }).join(', ');
...
function normalizeIndexDefinition(index) {
  if (index && typeof index['keys'] === 'object') {
    // Full form
