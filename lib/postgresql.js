...
function getTableStatus(model, cb) {
  ...
  this.query('SELECT t.relname AS "table", i.relname AS "index", ' +
    'a.attname AS "column" ' +
    'FROM pg_class t, pg_class i, pg_index ix, pg_attribute a ' +
    'WHERE t.oid = ix.indrelid AND i.oid = ix.indrelid AND ' +
...
PostgreSQL.prototype.addIndexes = function (model, actualIndexes, cb) {
  ...
  propNames.forEach(function (propName) {
    ...
    if (!found) {
      ...
      if (kind && type) {
        sql.push('CREATE ' + kind + ' INDEX ON ' + self.tableEscaped(model) + type + ' ( ' + pName + ' )');
      } else {
